...
Let's get down to business. This part is pretty straight forward, you just need to run:
```sh
npm create cloudflare@latest -- ystp
```

In the prompts you'll get just pick the following:
...
Before we move on with building _the thing_, let's update the name of our **DO** to accurately describe what it'll do, so name it `Relay`. Besides **updating the name of the class** add the following changes to your `wrangler.json`
```json
{
//...
  "migrations": [
	...
	async fetch(request, env, ctx): Promise<Response> {
		...
		if (request.method == 'GET' && request.headers.get('Upgrade') == 'websocket') {
			...
			if (request.url.endsWith('/send') || request.url.endsWith('/receive')) {
				let id: DurableObjectId = env.RELAY.idFromName('1234'); // We'll change this later.
				let stub = env.RELAY.get(id);
...
A more subtle change is that we've hardcoded the **DO** id we're going to use (as we can't use the pathname anymore, since `/send` and `/receive` would be instantiating and communicating with different **DO**s).
...
We can then accept the server side of the connection with `this.ctx.acceptWebSocket` and can pass in an optional array of tags we want to add to this connection. You'll shortly see why this is important.
